...
const EXERCISE_NAME_MAP: Record<string, string> = {
    ...
    '런지 (Lunge)': 'lunge',
    '크런치 (Crunch)': 'crunch',
    '크로스 런지 (Cross Lunge)': 'cross_lunge',
    '레그 레이즈 (Leg Raise)': 'leg_raise',
};
...
const AnalysisDemo = () => {
    ...
    const { mode, exercise: exerciseData } = (location.state as { mode?: string; exercise?: { title: string } }) || {};
        ...
        try {
            ...
            ws.onmessage = (event) => {
                ...
                if (data.type === 'init_success') {
                    ...
                    // 프레임 전송 시작 (20 FPS)
                    sendFrameIntervalRef.current = window.setInterval(sendFrame, 1000 / 20);
                    ...
                    <div className={styles.rightPanel}>
                        ...
                        <div className={styles.infoCard}>
                            ...
                            {aiStatus.is_warmup && (
                                <div className={styles.infoRow}>
                                    ...
                                    <span className={styles.infoValue}>
                                        {aiStatus.warmup_remaining ? `${aiStatus.warmup_remaining.toFixed(1)}초` : '준비 중'}
                                    </span>
                                </div>
                            )}
                            <div className={styles.infoRow}>
                            ...
                            {aiStatus.elapsed_seconds !== undefined && (
                                <div className={styles.infoRow}>
                                    ...
                                    <span className={styles.infoValue}>{aiStatus.elapsed_seconds}초</span>
                                </div>
                            )}
                        </div>
                        ...
                        <div className={styles.infoCard}>
                            ...
                            <div className={styles.feedbackContent}>
                                <p>{aiStatus.feedback_ko || '운동을 시작하면 실시간 피드백이 표시됩니다.'}</p>
                            </div>
                        </div>
                        ...
                        <div className={styles.infoCard}>
                            ...
                            <div className={styles.infoRow}>
                                ...
                                <span className={styles.infoValue}>{aiStatus.counters?.reps || 0}</span>
                            </div>
                            <div className={styles.infoRow}>
                                <span className={styles.infoLabel}>평균 점수:</span>
                                <span className={styles.infoValue}>
                                    {aiStatus.total_score ? `${aiStatus.total_score.toFixed(2)}점` : '-'}
                                </span>
                            </div>
                        </div>
                    </div>
